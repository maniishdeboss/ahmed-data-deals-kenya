...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  // Waxaan u oggolaanaynaa CORS si uu app-kaagu ugu xirmo
  ...
  try {
    ...
    const response = await fetch('https://tinypesa.com/api/v1/express/initialize', {
      ...
      // Halkan nambarkaaga rasmiga ah ayuu ku qoran yahay: 254725723383
      body: `amount=${amount}&msisdn=${phone}&account_no=254725723383`
    });

    const data = await response.json();

    // Waxaan hubinaynaa in TinyPesa ay si sax ah u aqbashay codsiga
